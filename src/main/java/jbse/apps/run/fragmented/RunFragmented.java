package jbse.apps.run.fragmented;
...
import jbse.apps.run.InitialHeapChecker;
 ...
 * At each step of the symbolic execution {@code Run} prints on a set of output
 * and error streams:
 * 
 * <ul>
 ...
 * </ul>
 * 
 * {@code Run} executes a trace until it reaches a stuck state; then, if it can
 * backtrack to some state, it asks the user via console whether it should
 ...
 * point is encountered. A {@code Run} session can be automatically dumped to
 * file for offline inspection.
 * 
 * @author Pietro Braione
 */
public final class RunFragmented {

	private Collection<State> finalStates = new ArrayList<State>();
	...
	private DecisionProcedureAlgorithms decisionProcedure = null; // TODO build run object during construction and make
																	// this final
	...
	/**
	 * The {@link DecisionProcedureGuidance}, whenever this method is chosen for
	 * stepping the {@link Engine}.
	 */
	...
	/**
	 * Counter for the number of analyzed traces that are safe (do not violate
	 * assertions).
	 */
	private long tracesSafe = 0;

	/**
	 * Counter for the number of analyzed traces that are unsafe (violate some
	 * assertion).
	 */
	private long tracesUnsafe = 0;

	/**
	 * Counter for the number of analyzed traces that are unmanageable (the symbolic
	 * executor is not able to execute them).
	 */
	...
	/**
	 * Counter for the number of analyzed traces that are out of scope and
	 * concretizable.
	 */
	 ...
	 * Constructor.
	 */
	public RunFragmented(RunParameters parameters) {
	...
	private class ActionsRun extends Runner.Actions {
		...
		/**
		 * Determines whether the stack size of the current state is below the maximum
		 * treshold for being printed.
		 * 
		 * @return {@code true} iff it is below the treshold.
		 */
		private boolean stackSizeAcceptable() {
			final State currentState = RunFragmented.this.engine.getCurrentState();
			return (RunFragmented.this.parameters.getStackDepthShow() == 0
					|| RunFragmented.this.parameters.getStackDepthShow() > currentState.getStackSize());
		}

		/**
		 * Displays the current state and prompts the user in case of interactive mode
		 * 
		 * @return {@code true} iff the user told to stop execution.
		 */
		private boolean printAndAsk() {
			if (this.endOfTraceMessage == null && this.traceKind != TraceTypes.CONTRADICTORY
					&& this.stackSizeAcceptable()) {
				try {
					final State currentState = RunFragmented.this.getCurrentState();
					RunFragmented.this.emitState(currentState, this.isBranch);
				} catch (UnexpectedInternalException e) {
					RunFragmented.this.err(ERROR_UNEXPECTED);
					RunFragmented.this.err(e);
					return true;
				}
			}
			...
			boolean stop = false;
			if (RunFragmented.this.parameters.getInteractionMode() == InteractionMode.STEP_BY_STEP) {
				final String ans = RunFragmented.this.in(PROMPT_SHOULD_STEP);
				if (ans.equals("x")) {
					stop = true;
				}
			}
		...
		public boolean atRoot() {
			RunFragmented.this.emitPrologue();

			// prints the state (all+bytecode and branches)
			boolean stop = false;
			if (RunFragmented.this.parameters.getStepShowMode() == StepShowMode.METHOD
					|| RunFragmented.this.parameters.getStepShowMode() == StepShowMode.SOURCE) {
				stop = printAndAsk();
			}
			return stop;
		}
		...
		public boolean atTraceStart() {
			...
			// exits if user wants
			boolean stop = false;
			if (RunFragmented.this.parameters.getStepShowMode() == StepShowMode.ALL
					|| RunFragmented.this.parameters.getStepShowMode() == StepShowMode.ROOT_BRANCHES_LEAVES) {
				stop = printAndAsk();
			}
		...
		@Override
		public void atEnd() {
			RunFragmented.this.emitEpilogue();
			super.atEnd();
		}
		...
		public void atTimeout() {
			if (RunFragmented.this.parameters.getShowWarnings()) {
				RunFragmented.this.log(WARNING_TIMEOUT);
			}
		}
		...
		@Override
		public boolean atCannotManageStateException(CannotManageStateException e) throws CannotManageStateException {
		...
		public boolean atStepPost() {
			...
			if (RunFragmented.this.parameters.getShowWarnings() && getEngine().someReferenceNotExpanded()) {
				final State currentState = RunFragmented.this.engine.getCurrentState();
				RunFragmented.this.log(currentState.getIdentifier() + " "
						+ getEngine().getNonExpandedReferencesOrigins() + WARNING_PARTIAL_REFERENCE_RESOLUTION);
			}
		...
		public boolean atSourceRowPost() {
			...
			boolean stop = false;
			if (RunFragmented.this.parameters.getStepShowMode() == StepShowMode.SOURCE) {
				stop = printAndAsk();
			}
			return stop;
		}
		...
		public boolean atTraceEnd() {
			try {
				...
				if (RunFragmented.this.parameters.getStepShowMode() == StepShowMode.ALL || // already shown
						RunFragmented.this.parameters.getStepShowMode() == StepShowMode.SOURCE || // already shown
						RunFragmented.this.parameters.getStepShowMode() == StepShowMode.METHOD || // already shown
						RunFragmented.this.parameters.getStepShowMode() == StepShowMode.NONE || // not to show
						!RunFragmented.this.parameters.getTracesToShow().contains(this.traceKind)) { // not to show
					...
					if (RunFragmented.this.parameters.getStepShowMode() == StepShowMode.SUMMARIES) {
						State initialRefined = RunFragmented.this.engine.getInitialState();
						initialRefined.refine(currentState);
						RunFragmented.this.emitState(initialRefined, true);
						RunFragmented.this.out("\n===\n");
					}
					// prints the leaf (stuck) state
					RunFragmented.this.emitState(currentState,
							(RunFragmented.this.parameters.getStepShowMode() == StepShowMode.LEAVES
									|| RunFragmented.this.parameters.getStepShowMode() == StepShowMode.SUMMARIES));
					RunFragmented.this.finalStates.add(currentState);
				}
				...
				case OUT_OF_SCOPE:
					...
					break;
				case UNMANAGEABLE:
					++RunFragmented.this.tracesUnmanageable;
					// this.endOfTraceMessage already set
					counterKind = null;
					break;
				case CONTRADICTORY:
				...
				if (RunFragmented.this.parameters.getShowWarnings()) {
					RunFragmented.this.log(currentState.getIdentifier() + this.endOfTraceMessage);
				}
				if (RunFragmented.this.parameters.getDoConcretization()) {
		...
		public boolean atBacktrackPre() {
			...
			boolean stop = false;
			if (RunFragmented.this.parameters.getInteractionMode() == InteractionMode.PROMPT_BACKTRACK) {
				final String ans = RunFragmented.this.in(PROMPT_SHOULD_BACKTRACK);
				if (ans.equals("x")) {
					stop = true;
				}
			}
		...
		@Override
		public boolean atDecisionException(DecisionException e) throws DecisionException {
			RunFragmented.this.err(ERROR_ENGINE_DECISION_PROCEDURE);
			RunFragmented.this.err(e);
			return super.atDecisionException(e);
		}

		@Override
		public boolean atEngineStuckException(EngineStuckException e) throws EngineStuckException {
			RunFragmented.this.err(ERROR_ENGINE_STUCK);
			RunFragmented.this.err(e);
			return super.atEngineStuckException(e);
		}

		@Override
		public boolean atClasspathException(ClasspathException e) throws ClasspathException {
			RunFragmented.this.err(ERROR_BAD_CLASSPATH);
			RunFragmented.this.err(e);
			return super.atClasspathException(e);
		}

		@Override
		public boolean atDecisionBacktrackException(DecisionBacktrackException e) throws DecisionBacktrackException {
			RunFragmented.this.err(ERROR_ENGINE_DECISION_PROCEDURE);
			RunFragmented.this.err(e);
		...
		@Override
		public boolean atCannotBacktrackException(CannotBacktrackException e) throws CannotBacktrackException {
			RunFragmented.this.err(ERROR_UNEXPECTED);
			RunFragmented.this.err(e);
		...
		private void checkFinalStateIsConcretizable(CounterKind ctr) {
			...
			final boolean concretizable = RunFragmented.this.checker.checkHeap(false);
			final long elapsedTime = System.currentTimeMillis() - startTime;
			RunFragmented.this.elapsedTimeConcretization += elapsedTime;
			if (concretizable) {
				if (ctr == CounterKind.INC_OUT_OF_SCOPE) {
					++RunFragmented.this.tracesConcretizableOutOfScope;
				} else if (ctr == CounterKind.INC_SAFE) {
					++RunFragmented.this.tracesConcretizableSafe;
				} else { // ctr == CounterKind.INC_UNSAFE
					++RunFragmented.this.tracesConcretizableUnsafe;
				}
			}
			if (RunFragmented.this.parameters.getShowWarnings()) {
				final State currentState = RunFragmented.this.engine.getCurrentState();
	 ...
	 * Runs the method.
	 * 
	 * @return an {@code int} value representing an error code, {@code 0} if
	 *         everything went ok, {@code 1} if the cause of the error was external
	 *         (inputs), {@code 2} if the cause of the error was internal (bugs).
	 */
	public int run() {
		// sets up this object
		int retVal = build();
		...
		try {
			this.runner.run();
				...
				| CannotBacktrackException e) {
			// already reported
			retVal = 1;
		} catch (ThreadStackEmptyException | ContradictionException | FailureException
	 ...
	 * 
	 * @param t
	 *            the {@link Throwable} to be printed.
	 */
	public void err(Throwable t) {
		IO.printException(this.err, t);
	}

	private static final String COMMANDLINE_LAUNCH_Z3 = System.getProperty("os.name").toLowerCase().contains("windows")
			? " /smt2 /in /t:10"
			: " -smt2 -in -t:10";
	...
	private int build() {
		...
		// TODO rethrow exception rather than returning an int, and centralize logging
		// in the receiver
	...
	/**
	 * Returns the engine's initial state. Convenience for formatter and decision
	 * procedure creation.
	 * 
	...
	/**
	 * Returns the engine's initial state. Convenience for decision procedure
	 * creation.
	 * 
	 * @return the initial {@link State}.
	 */
	...
	/**
	 * Returns the decision procedure's current model or {@code null}. Convenience
	 * for formatter creation.
	 * 
	 * @return a
	 *         {@link Map}{@code <}{@link PrimitiveSymbolic}{@code ,}{@link Simplex}{@code >}
	 *         or {@code null}
	 */
	 ...
	 * 
	 * @throws CannotBuildFormatterException
	 *             upon failure.
	 */
		...
		if (type == StateFormatMode.FULLTEXT) {
			this.formatterBranches = new StateFormatterText(this.parameters.getSourcePath()) {
				...
				public void formatState(State s) {
					...
					this.output += banner(s.getIdentifier() + "[" + s.getSequenceNumber() + "]", true);
					this.output += LINE_SEP; // gutter
					super.formatState(s);
				}
			};
			this.formatterOthers = new StateFormatterText(this.parameters.getSourcePath()) {
				...
				public void formatState(State s) {
					this.output += LINE_SEP; // gutter
					this.output += banner(s.getIdentifier() + "[" + s.getSequenceNumber() + "]", false);
					this.output += LINE_SEP; // gutter
					super.formatState(s);
				}
			};
			...
			this.formatterBranches = this.formatterOthers = new StateFormatterJUnitTestSuite(this::getInitialState,
					this::getModel);
		} else {
	 ...
	 * 
	 * @param s
	 *            a {@link String} around which the banner is built, or {@link null}
	 *            for a clean banner.
	 * @return the banner.
	 */
	 ...
	 *             upon failure (cannot instantiate rewriters).
	 */
	...
	/**
	 * Creates the decision procedures in {@code this.decisionProcedure} and
	 * {@code this.decisionProcedureConcretization}.
	 * 
	 * @param calc
	 *            a {@link CalculatorRewriting}.
	 * @throws CannotBuildDecisionProcedureException
	 *             upon failure.
	 */
	private void createDecisionProcedure(CalculatorRewriting calc) throws CannotBuildDecisionProcedureException {
		...
		final boolean needHeapCheck = (this.parameters.getUseConservativeRepOks()
				|| this.parameters.getDoConcretization());
		...
		if (this.parameters.getUseConservativeRepOks()) {
			...
			final DecisionProcedureConservativeRepOk dec = new DecisionProcedureConservativeRepOk(core, calc,
					checkerParameters, this.parameters.getConservativeRepOks());
		...
		if (this.parameters.isGuided()) {
			...
			try {
				if (this.parameters.getGuidanceType() == GuidanceType.JBSE) {
					this.guidance = new DecisionProcedureGuidanceJBSE(core, calc, guidanceDriverParameters,
							this.parameters.getMethodSignature());
				} else if (this.parameters.getGuidanceType() == GuidanceType.JDI) {
					this.guidance = new DecisionProcedureGuidanceJDI(core, calc, guidanceDriverParameters,
							this.parameters.getMethodSignature());
				} else {
		...
		// sets the result
		this.decisionProcedure = ((core instanceof DecisionProcedureAlgorithms) ? (DecisionProcedureAlgorithms) core
	 ...
	 * 
	 * @param decisionProcedureConcretization
	 *            the {@link DecisionProcedureAlgorithms} to be used by the heap
	 *            checker.
	 */
	private void createHeapChecker(DecisionProcedureAlgorithms decisionProcedureConcretization) {
		if (this.parameters.getDoConcretization()) {
			...
			this.checker = new InitialHeapChecker(checkerParameters, ConcretizationCheck.class,
					this.parameters.getConcretizationMethods());
	 ...
	 * 
	 * @param s
	 *            the {@link State} to be emitted.
	 * @param isRootBranch
	 *            {@code true} iff {@code s} is at a branch point.
	 */
	private void emitState(State s, boolean isRootBranch) {
		final Formatter f = (isRootBranch ? this.formatterBranches : this.formatterOthers);
		f.cleanup();
		f.formatState(s);
		outNoBreak(f.emit());
	}
	...
	private void printFinalStats() {
		...
		final long tracesContradictory = this.runner.getTracesTotal() - this.tracesSafe - this.tracesUnsafe
				- this.runner.getTracesOutOfScope() - this.tracesUnmanageable;
		log(MSG_END_STATES + this.engine.getAnalyzedStates() + ", " + MSG_END_TRACES_TOT + this.runner.getTracesTotal()
				+ ", " + MSG_END_TRACES_SAFE + this.tracesSafe
				+ (RunFragmented.this.parameters.getDoConcretization()
				...
				+ (RunFragmented.this.parameters.getDoConcretization()
						...
						: "")
				+ ", " + MSG_END_TRACES_VIOLATING_ASSUMPTION + tracesContradictory + ", " + MSG_END_TRACES_UNMANAGEABLE
				+ this.tracesUnmanageable + ".");
		final long elapsedTimeDecisionProcedure = (this.timer == null ? 0 : this.timer.getTime());
		log(MSG_END_ELAPSED + Util.formatTime(elapsedTime) + ", " + MSG_END_SPEED
				+ this.engine.getAnalyzedStates() * 1000 / elapsedTime + " states/sec"
				+ (RunFragmented.this.parameters.getDoConcretization()
	...
	/**
	 * Closes this {@link RunFragmented} object.
	 * 
	 * @return an {@code int} value representing an error code.
	 */
	private int close() {
	...
	/** Message: welcome. */
	private static final String MSG_WELCOME_TXT = "This is the " + JBSE.NAME + "'s Run Tool (" + JBSE.ACRONYM + " v."
			+ JBSE.VERSION + ").";
	...
	/**
	 * Warning: a method call cannot be treated as returning an uninterpreted
	 * function value.
	 */
	...
	/**
	 * Error : unable to initialize engine, because unable to initialize the
	 * decision procedure.
	 */
	...
	/**
	 * Error: unable to initialize engine, because unable to create the initial
	 * state.
	 */
	...
	/**
	 * Error: unable to quit engine, because unable to quit the decision procedure.
	 */
	...
	public long getTracesSafe() {
		return this.tracesSafe;
	}

	public long getTracesUnsafe() {
		return this.tracesUnsafe;
	}

	public long getTracesConcretizableSafe() {
		return this.tracesConcretizableSafe;
	}

	public long getTracesConcretizableUnsafe() {
		return this.tracesConcretizableUnsafe;
	}

	public long getTracesConcretizableOutOfScope() {
		return this.tracesConcretizableOutOfScope;
	}
